...
@interface RPAreaDataManager ()

@property (nonatomic, strong) FMDatabase *db;

@end

@implementation RPAreaDataManager

static RPAreaDataManager *manager = nil;
+ (RPAreaDataManager *)shareInstance {
...
- (void)searchCityData:(NSString *)searchObject result:(void (^)(NSMutableArray *result))result {
    ...
    if (resultArray.count == 0) {
        ...
        if (resultArray.count == 0) {
            ...
            //统一在数组中传字典是为了RPSearchView解析数据时方便
            if (resultArray.count == 0) {
