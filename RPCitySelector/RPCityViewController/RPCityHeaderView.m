...
@interface RPCityHeaderView ()<UISearchBarDelegate>

@property (nonatomic, strong) UILabel *currentCityLabel;
@property (nonatomic, strong) RPButton *button;
@property (nonatomic, strong) UISearchBar *searchBar;

@end

@implementation RPCityHeaderView
...
- (void)addSearchBar {
    ...
    separator.backgroundColor = RPRGBAColor(155, 155, 155, 0.5);
...
- (void)addRPButton {
    self.button = [[RPButton alloc] initWithFrame:CGRectMake(self.frame.size.width - 95, self.frame.size.height - 31, 75, 21)];
    [_button addTarget:self action:@selector(touchUpRPButtonEnevt:) forControlEvents:UIControlEventTouchUpInside];
    _button.imageName = @"down_arrow_icon1";
    _button.title = @"选择区县";
    _button.titleColor = RPRGBAColor(155, 155, 155, 1.0);
    [self addSubview:_button];
}

- (void)touchUpRPButtonEnevt:(RPButton *)sender {
