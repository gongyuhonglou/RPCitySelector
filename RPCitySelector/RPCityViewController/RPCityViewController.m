...
RPLocationDelegate,
RPCityHeaderViewDelegate,
RPSearchViewDelegate>
...
@property (nonatomic, strong) RPCityTableViewCell *cell;
@property (nonatomic, strong) RPCityHeaderView *headerView;
@property (nonatomic, strong) RPAreaDataManager *manager;
@property (nonatomic, strong) RPLocation *locationManager;
@property (nonatomic, strong) RPSearchView *searchView;
/** 最近访问的城市*/
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(chooseCityWithName:) name:RPCityTableViewCellDidChangeCityNotification object:nil];
    
    [self.view addSubview:self.rootTableView];
    self.rootTableView.tableHeaderView = self.headerView;
    
    [self backBarButtonItem];
    [self initWithRPAreaDataManaager];
...
/// 初始化数据库，获取所有“市”级城市名称
- (void)initWithRPAreaDataManaager {
...
- (RPCityHeaderView *)headerView {
    if (!_headerView) {
        _headerView = [[RPCityHeaderView alloc] initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width, 80)];
...
- (RPSearchView *)searchView {
    if (!_searchView) {
        CGRect frame = [UIScreen mainScreen].bounds;
        _searchView = [[RPSearchView alloc] initWithFrame:CGRectMake(0, 104, frame.size.width, frame.size.height  - 104)];
...
- (UITableView *)rootTableView {
    if (!_rootTableView) {
        ...
        [_rootTableView registerClass:[RPCityTableViewCell class] forCellReuseIdentifier:@"cityCell"];
...
#pragma mark --- RPCityHeaderViewDelegate
...
#pragma mark - RPSearchViewDelegate
...
- (void)dealloc {
    NSLog(@"RPCityViewController dealloc");
}
